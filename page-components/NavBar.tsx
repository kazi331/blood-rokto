  ...
  return (
    <nav className='main-nav shadow'>
      <div className="container mx-auto flex flex-wrap justify-between items-center px-2 md:px-4 lg:px-16 xl:px-0 font-medium text-lg">
        ...
        {/* mobile menu  */}
        <div onClick={(e) => setToggleMenu(false)} className={`${toggleMenu ? 'bottom-0' : '-bottom-full p-0 m-0 overflow-hidden'} flex top-100 h-screen w-full p-4 py-8 transition-all duration-300 justify-center items-end md:hidden fixed left-0  bg-gray-900 bg-opacity-80 backdrop-blur-lg text-gray-200 z-10 `}>
        ...
        <div className="fixed bottom-10 right-6 z-20">
          <button className='block md:hidden outline-none active:outline-none text-white bg-gray-800 bg-opacity-60 backdrop-blur-lg p-2 text-2xl rounded-3xl shadow-sm shadow-gray-900' onClick={() => setToggleMenu(!toggleMenu)}>
