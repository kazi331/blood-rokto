  ...
  return (
    <nav className='main-nav '>
      <div className="container mx-auto flex flex-wrap justify-between items-center px-2 md:px-4 font-medium text-lg">
        ...
        {/* mobile menu  */}
        <div onClick={(e) => setToggleMenu(false)} className={`${toggleMenu ? 'min-h-screen w-full' : 'w-0 h-0 overflow-hidden'} fixed top-0 left-0 lg:hidden flex bg-gray-900 bg-opacity-80 z-10 backdrop-blur-sm `}>
          <div className={`${toggleMenu ? 'ml-0' : '-ml-[100vw]'} flex flex-col w-[50vw] max-w-sm transition-all duration-700 bg-gray-900 bg-opacity-95 shadow-black shadow-xl`}>
            <div className="bg-primary bg-opacity-50 text-gray-200 p-6 text-3xl underline underline-offset-4">  Blood Ai</div>
            <div className='flex flex-col p-6'>{pages.map((page, i) => <Link key={i} href={page.slug} className='font-thin text-gray-400 hover:text-white rounded py-1 px-2 transition min-w-max '>{page.label}</Link>)}</div>
          </div>
        </div>
